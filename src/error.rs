...
impl fmt::Display for EndDiagnostic {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        comma_separated.push_str(format!("({}, {}), ", &self.stack[self.stack.len() - 1].0.as_str(), &self.stack[self.stack.len() - 1].1).as_str());

        write!(f, "Unexpected ending. Index:{}, Stack: {}", self.index, comma_separated)
    }
}
